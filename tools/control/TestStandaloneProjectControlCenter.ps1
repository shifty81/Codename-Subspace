...
$paths=@('SubspaceTools.cmd','SubspaceTools.ps1','tools\control\StandaloneProjectControlCenter.ps1','tools\control\StandalonePatchEngine.ps1','tools\control\WriteQualityGateRecord.ps1','tools\control\SubspaceControlCenter.ps1','tools\control\GitBranchManager.ps1','project.control.json')
...
foreach($s in @('FULL QUALITY GATE / CERTIFY GREEN','COMMIT + PUSH CURRENT GREEN','Apply this patch now? [Y/N]','git-commit-green','git-push','RequireNoPendingPatch','Assert-StartupIntakePolicy','Full Gate never auto-applies','[STEP] COMMIT: verify or create the exact certified GREEN commit.','[STEP] PUSH + REMOTE VERIFY: publish current branch and verify origin matches local HEAD.','[PASS] REMOTE VERIFY: certified GREEN source is committed and present on origin.','Branches / switch / create / toggle','GitBranchManager.ps1','RequireBranchGateFresh','branch-switch-state.json')){T ("PCC contract: $s") $pcc.Contains($s)}
$branch=Get-Content -LiteralPath (Join-Path $Root 'tools\control\GitBranchManager.ps1') -Raw
foreach($s in @('SAFE BRANCH MANAGER','Assert-WorkingTreeClean','Assert-NoPendingPatch','Toggle-PreviousBranch','Switch-RemoteTracking','Create-NewBranch','git switch -','branch-switch-state.json','last-green-quality-gate.json','No auto-stash, reset, checkout-force, branch deletion, merge, rebase, or force push.')){T ("branch contract: $s") $branch.Contains($s)}
T 'branch manager forbids force checkout/reset' (-not $branch.Contains('checkout -f') -and -not $branch.Contains('reset --hard') -and -not $branch.Contains('push --force'))
