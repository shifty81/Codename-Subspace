> **Native runtime note (2026-08-30):** Assimp.NET/.NET references below describe the retired prototype pipeline. Current Subspace uses the native C++ asset/runtime path; use the root Full Gate and native OBJ/import tooling.

# Ship Module 3D Models

This directory contains 3D model files for modular ship components.

## Native C++ Pass340 Visual Authority

The shipping native C++ renderer now discovers every `.obj` in this module directory at load and builds a deterministic procedural visual catalog from the available authored pieces. The current runtime generates role-aware ship recipes in memory rather than duplicating mesh files. Adding a compatible OBJ here therefore expands the visual source vocabulary automatically; gameplay/module statistics remain owned by native C++ gameplay data, not by the generated visual recipe.

`assets/Models/ships/modules` is the repository authoring source when present. `GameData/Assets/Models/ships/modules` is the packaged-runtime fallback. Legacy C# references below are retained as historical documentation only and are not shipping runtime authority.
...
1. Build the project: run `./SubspaceTools.ps1` and select **FULL GATE** on Windows
