...
enum class ShipClass {
    ...
    Miner,
    Explorer,
    // Pass481 appends new size authorities instead of inserting them so old
    // serialized enum values remain stable. Miner/Explorer/Freighter remain
    // compatibility hull labels while new progression separates size from role.
    Shuttle,
    Battlecruiser,
    Dreadnought,
    IndustrialCapital,
    // Pass615 normalization: canonical strategic hull-size authority. Appended
    // to preserve every pre-existing serialized enum value.
    Capital
};
...
enum class ShipRole {
    ...
    Construction,
    Refining,
    // Pass615 normalization: role-specialization vocabulary used by the
    // faction hull-family/PCG layer. Appended to keep legacy serialization stable.
    GeneralCombat,
    Escort,
    Scout,
    Carrier,
