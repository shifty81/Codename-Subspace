...
public:
    ...
    PlayerControlSystem*       GetPlayerControlSystem()       { return _playerControlSystem; }
    const PlayerControlSystem* GetPlayerControlSystem() const { return _playerControlSystem; }

    StrategicCamera&       GetStrategicCamera()       { return _strategicCamera; }
    const StrategicCamera& GetStrategicCamera() const { return _strategicCamera; }

    RenderingConfiguration&       GetRenderingConfiguration()       { return _renderingConfiguration; }
    const RenderingConfiguration& GetRenderingConfiguration() const { return _renderingConfiguration; }

    NativeRuntimeServices&       GetRuntimeServices()       { return _runtimeServices; }
    const NativeRuntimeServices& GetRuntimeServices() const { return _runtimeServices; }

    /// Update the drawable viewport used by native UI/render command generation.
...
private:
    ...
    StrategicCamera  _strategicCamera;
    RenderingConfiguration _renderingConfiguration;
    NativeRuntimeServices _runtimeServices;
    UISystem*        _uiSystem = nullptr;   // non-owning; owned by EntityManager
    PlayerControlSystem* _playerControlSystem = nullptr; // non-owning; owned by EntityManager
