...
struct CombatComponent : public IComponent {
    ...
    float armorRating = 0.0f;      // per-hit mitigation before armor integrity loss
    float maxArmorHP = 100.0f;
    float currentArmorHP = 100.0f;
    float maxHullHP = 100.0f;
    float currentHullHP = 100.0f;
    ...
    /// Configure ship durability from authored/module-derived capacities.
    /// When preserveDamageFraction is true, refits/captures retain the same
    /// percentage damage instead of magically repairing the ship.
    void ConfigureDurability(float shieldHP, float armorHP, float hullHP,
                             float energyHP, bool preserveDamageFraction = false);

    float ShieldFraction() const;
    float ArmorFraction() const;
    float HullFraction() const;
    float PowerFraction() const;
    bool IsDestroyed() const { return currentHullHP <= 0.0f; }
};
...
public:
    ...
    /// Apply damage to a target: shields first, then armor integrity, then hull.
    /// Returns actual durability removed across those layers.
