...
bool InventoryComponent::AddItem(const InventoryItem& item) {
    ...
    for (auto& slot : _slots) {
        ...
        if (item.uniqueInstance || slot.item.uniqueInstance) continue;
...
ComponentData InventoryComponent::Serialize() const {
    ...
    for (size_t i = 0; i < _slots.size(); ++i) {
        ...
        cd.data[prefix + "instanceId"]   = item.instanceId;
        ...
        cd.data[prefix + "uniqueInstance"] = item.uniqueInstance ? "1" : "0";
        cd.data[prefix + "sourceModuleId"] = item.sourceModuleId;
        cd.data[prefix + "iconKey"] = item.iconKey;
        cd.data[prefix + "quality"] = std::to_string(item.quality);
        cd.data[prefix + "condition"] = std::to_string(item.condition);
        cd.data[prefix + "blueprintId"] = item.blueprintId;
        cd.data[prefix + "blueprintFragmentIndex"] = std::to_string(item.blueprintFragmentIndex);
        cd.data[prefix + "blueprintFragmentsRequired"] = std::to_string(item.blueprintFragmentsRequired);
        cd.data[prefix + "statCount"] = std::to_string(item.stats.size());
        for (size_t si=0; si<item.stats.size(); ++si) {
            cd.data[prefix + "stat" + std::to_string(si) + "_name"] = item.stats[si].first;
            cd.data[prefix + "stat" + std::to_string(si) + "_value"] = std::to_string(item.stats[si].second);
        }
        cd.data[prefix + "affixCount"] = std::to_string(item.affixes.size());
        for (size_t ai=0; ai<item.affixes.size(); ++ai) cd.data[prefix + "affix" + std::to_string(ai)] = item.affixes[ai];
        ++usedCount;
    }
...
void InventoryComponent::Deserialize(const ComponentData& data) {
    ...
    for (int i = 0; i < usedCount; ++i) {
        ...
        slot.item.instanceId   = getStr(prefix + "instanceId");
        ...
        slot.item.uniqueInstance = getStr(prefix + "uniqueInstance") == "1";
        slot.item.sourceModuleId = getStr(prefix + "sourceModuleId");
        slot.item.iconKey = getStr(prefix + "iconKey");
        slot.item.quality = getFloat(prefix + "quality", 1.0f);
        slot.item.condition = getFloat(prefix + "condition", 1.0f);
        slot.item.blueprintId = getStr(prefix + "blueprintId");
        slot.item.blueprintFragmentIndex = getInt(prefix + "blueprintFragmentIndex", 0);
        slot.item.blueprintFragmentsRequired = getInt(prefix + "blueprintFragmentsRequired", 0);
        const int statCount = getInt(prefix + "statCount", 0);
        for (int si=0; si<statCount; ++si) slot.item.stats.push_back({getStr(prefix + "stat" + std::to_string(si) + "_name"), getFloat(prefix + "stat" + std::to_string(si) + "_value", 0.0f)});
        const int affixCount = getInt(prefix + "affixCount", 0);
        for (int ai=0; ai<affixCount; ++ai) slot.item.affixes.push_back(getStr(prefix + "affix" + std::to_string(ai)));
    }
