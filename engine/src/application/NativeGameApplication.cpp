...
void NativeGameApplication::HandleShipyardTransformHotkeys()
{
    ...
    if(tool==ShipyardTransformTool::Move){
        ...
        if(space==ShipyardTransformSpace::View){
            ...
            float runtimeYaw=0.0f;
            if(!_standaloneShipyard){if(const auto* player=_engine.GetEntityManager().GetComponent<PhysicsComponent>(_playerEntity))runtimeYaw=player->rotation.z;}
            const float rootYaw=EditorTransformSpaceSystem::RenderedRootYawRadians(runtimeYaw,_shipBuilder.Recipe().forwardVisualYawDegrees);
            local=EditorTransformSpaceSystem::WorldToAssemblyDelta(world,rootYaw);
            if(socketEdit)if(const auto* placed=_shipBuilder.SelectedPlacedModule())local=EditorTransformSpaceSystem::AssemblyToModuleLocalDelta(local,*placed);
        }else if(space==ShipyardTransformSpace::Ship&&socketEdit){
            if(const auto* placed=_shipBuilder.SelectedPlacedModule())local=EditorTransformSpaceSystem::AssemblyToModuleLocalDelta(local,*placed);
        }else if(space==ShipyardTransformSpace::Local&&!socketEdit){
            if(_shipBuilder.Model().dragPreview.staged){
                constexpr float kDegToRad=3.14159265358979323846f/180.0f;
                const float a=_shipBuilder.Model().dragPreview.ghost.yawDegrees*kDegToRad,c=std::cos(a),sn=std::sin(a);
                local={x*c-y*sn,x*sn+y*c,z};
            }else if(const auto* placed=_shipBuilder.SelectedPlacedModule()){
                constexpr float kDegToRad=3.14159265358979323846f/180.0f;
                const float a=placed->yawDegrees*kDegToRad,c=std::cos(a),sn=std::sin(a);
                local={x*c-y*sn,x*sn+y*c,z};
            }
        }
        if(_shipBuilder.Model().dragPreview.staged){_shipBuilder.TranslateStagedPlacement(local,fine);}
        else if(socketEdit){if(_shipBuilder.TranslateSelectedSocket(local,fine))_shipBuilder.CommitSocketTransform();}
    ...
    if(tool==ShipyardTransformTool::Rotate){
        ...
        if(std::fabs(delta.x)+std::fabs(delta.y)+std::fabs(delta.z)>0.0001f){
            if(_shipBuilder.Model().dragPreview.staged)_shipBuilder.RotateStagedPlacement(delta,fine);
            else if(_shipBuilder.Model().inspectorTab==ShipyardInspectorTab::Sockets){if(_shipBuilder.RotateSelectedSocket(delta,fine))_shipBuilder.CommitSocketTransform();}
    ...
    if(tool==ShipyardTransformTool::Scale){
        ...
        if(std::fabs(amount)>0.0001f){if(_shipBuilder.Model().dragPreview.staged)_shipBuilder.ScaleStagedPlacement(amount,fine);else if(_shipBuilder.ScaleSelected({amount,amount,amount},fine))_shipBuilder.CommitTransform();}
    }
}
...
{
    std::size_t projectToolIndex=0;
    if(_shipBuilder.ConsumeProjectToolRequest(&projectToolIndex)){
        const auto snapshot=_shipBuilder.Model().projectTools;
        if(projectToolIndex<snapshot.commands.size()){
            const auto command=snapshot.commands[projectToolIndex];
            const auto preview=ForgeWorkspaceSystem::CommandPreview(command);
            const auto root=snapshot.projectRoot.string();
            std::thread([root,preview](){
#ifdef _WIN32
                const std::string line="cd /d \""+root+"\" && "+preview;
#else
                const std::string line="cd \""+root+"\" && "+preview;
#endif
                (void)std::system(line.c_str());
            }).detach();
            Logger::Instance().Info("Shipyard","Queued project-control command: "+command.key+" / "+preview);
        }
    }
...
void NativeGameApplication::HandleGlobalActions()
{
    ...
    if(_standaloneShipyard){
        ...
        if(_window.ConsumePrimaryPress(pressX,pressY)){
            ...
            const bool dockCaptured=_shipyardDockPointer.Begin(_shipBuilder.Model().dockWorkspace,
                _window.GetWidth(),std::max(1,static_cast<int>(dockLayout.statusY)),dockLayout.viewportTop,pressX,pressY);
            if(!dockCaptured){
        ...
        if(_window.ConsumePrimaryDragDelta(dragX,dragY)){
            if(_shipyardDockPointer.Active()){
                const auto dockLayout=ShipyardBuilderSystem::Layout(_window.GetWidth(),_window.GetHeight());
                _shipyardDockPointer.Drag(_shipBuilder.MutableDockWorkspace(),dragX,dragY,
            ...
            if(_shipyardCatalogPointerDrag){
                ...
                const float rootYaw=EditorTransformSpaceSystem::RenderedRootYawRadians(0.0f,_shipBuilder.Recipe().forwardVisualYawDegrees);
        ...
        float sx=0.0f,sy=0.0f;if(_window.ConsumePrimaryClick(sx,sy)){if(!_shipyardDockSuppressClick){const auto c=ShipyardBuilderSystem::HitTest(_shipBuilder.Model(),_window.GetWidth(),_window.GetHeight(),sx,sy);if(c.command!=ShipyardBuilderCommand::None)ActivateShipyardControl(c.command,c.value);else if(!_shipBuilder.Model().dragPreview.staged &&
                !ShipyardDockPointerSystem::CoversFloatingPanel(_shipBuilder.Model().dockWorkspace,
                    _window.GetWidth(),std::max(1,static_cast<int>(ShipyardBuilderSystem::Layout(_window.GetWidth(),_window.GetHeight()).statusY)),
                    ShipyardBuilderSystem::Layout(_window.GetWidth(),_window.GetHeight()).viewportTop,sx,sy)){
                const int picked=NativeBattlefieldRenderer::PickShipyardModule(_shipBuilder.Model().catalog,_shipBuilder.Recipe(),_engine.GetStrategicCamera(),_window.GetWidth(),_window.GetHeight(),sx,sy,0,0,0,.24f,.22f,true);if(picked>=0)_shipBuilder.Activate(ShipyardBuilderCommand::SelectPlaced,picked);}}}
        _shipyardDockSuppressClick=false;
    ...
    if(_workspace.Mode()==SandboxWorkspaceMode::ShipBuilder&&_shipBuilder.IsInitialized()){
        ...
        if(_window.ConsumePrimaryPress(px,py)){
            const auto dockLayout=ShipyardBuilderSystem::Layout(_window.GetWidth(),_window.GetHeight());
            const bool dockCaptured=_shipyardDockPointer.Begin(_shipBuilder.Model().dockWorkspace,
                _window.GetWidth(),std::max(1,static_cast<int>(dockLayout.statusY)),dockLayout.viewportTop,px,py);
            if(!dockCaptured){
        ...
        float dx=0,dy=0;if(_window.ConsumePrimaryDragDelta(dx,dy)){
            if(_shipyardDockPointer.Active()){
                const auto dockLayout=ShipyardBuilderSystem::Layout(_window.GetWidth(),_window.GetHeight());
                _shipyardDockPointer.Drag(_shipBuilder.MutableDockWorkspace(),dx,dy,
                    _window.GetWidth(),std::max(1,static_cast<int>(dockLayout.statusY)),dockLayout.viewportTop);
            } else {
            if(_shipyardCatalogPointerCandidate&&!_shipyardCatalogPointerDrag){
                const float ddx=_window.GetPointerX()-_shipyardCatalogPressX,ddy=_window.GetPointerY()-_shipyardCatalogPressY;
        ...
        float rx=0,ry=0;if(_window.ConsumePrimaryRelease(rx,ry)){
            if(_shipyardDockPointer.Active()){
                const auto dockLayout=ShipyardBuilderSystem::Layout(_window.GetWidth(),_window.GetHeight());
