...
ShipyardModuleRecord Module(const char* id,float halfWidth=1.5f,float halfLength=2.0f,float halfHeight=1.0f,bool primary=true){
    ...
    r.moduleClass=primary?ShipyardModuleClass::Hull:ShipyardModuleClass::Adapter;r.semantic=primary?ShipyardModuleSemantic::HullMid:ShipyardModuleSemantic::StructuralFrame;r.size=ShipyardModuleSize::M;
    r.builderCategory=primary?ShipyardPartCategory::Hull:ShipyardPartCategory::Adapter;r.partRole=primary?ShipyardPartRole::PrimaryHull:ShipyardPartRole::StructuralFrame;r.primaryHull=primary;r.generatorEligible=true;r.placementRole="STRUCTURAL";
    return r;
}

ProceduralShipVisualRecipe SpanRecipe(const ShipyardModuleRecord& hull,const ShipyardModuleRecord& structural,int count,float minCenter,float maxCenter){
    ProceduralShipVisualRecipe r;r.recipeId="integrity";
    for(int i=0;i<count;++i){VisualModulePlacement p;p.moduleId=(i==0?hull.source.moduleId:structural.source.moduleId);const float t=count<=1?0.5f:static_cast<float>(i)/static_cast<float>(count-1);p.y=minCenter+(maxCenter-minCenter)*t;r.modules.push_back(p);}
    return r;
}
}
...
int main(){
    ...
    auto tiny=SpanRecipe(hull,frame,2,-2.0f,2.0f);const auto tinyBefore=ShipClassGenerationAuthoritySystem::MeasureLengthMeters(tiny,catalog);
    ...
    auto oversized=SpanRecipe(hull,frame,6,-30.0f,30.0f);auto lowCorrection=ShipClassGenerationAuthoritySystem::Resolve(oversized,catalog,ShipClass::Frigate,U::XS);
    ...
    auto undersized=SpanRecipe(hull,frame,6,-13.0f,13.0f);auto highCorrection=ShipClassGenerationAuthoritySystem::Resolve(undersized,catalog,ShipClass::Frigate,U::XS);
    ...
    auto valid=SpanRecipe(hull,frame,6,-18.0f,18.0f);auto validReport=ShipClassGenerationAuthoritySystem::Resolve(valid,catalog,ShipClass::Frigate,U::XS);
    ...
    auto sparse=SpanRecipe(hull,frame,5,-18.0f,18.0f);auto sparseReport=ShipClassGenerationAuthoritySystem::Resolve(sparse,catalog,ShipClass::Frigate,U::XS);
    Check(!sparseReport.valid,"1108 class minimum module topology is a hard generation constraint");
    auto dense=SpanRecipe(hull,frame,23,-18.0f,18.0f);auto denseReport=ShipClassGenerationAuthoritySystem::Resolve(dense,catalog,ShipClass::Frigate,U::XS);
    ...
    auto rotated=SpanRecipe(hull,frame,1,0,0);rotated.modules.front().yawDegrees=90.0f;auto rb=ShipClassGenerationAuthoritySystem::MeasureBoundsMeters(rotated,catalog);
    Check(rb.valid&&Near(rb.size.x,4.0f)&&Near(rb.size.y,3.0f),"1111 physical measurement honors transformed module orientation");

    auto needleModule=Module("needle",.2f,2.0f,1.0f);auto needleFrame=Module("needle_frame",.2f,2.0f,1.0f,false);std::vector<ShipyardModuleRecord> needleCatalog{needleModule,needleFrame};auto needle=SpanRecipe(needleModule,needleFrame,6,-18,18);
    ...
    auto flatModule=Module("flat",1.5f,2.0f,.10f);auto flatFrame=Module("flat_frame",1.5f,2.0f,.10f,false);std::vector<ShipyardModuleRecord> flatCatalog{flatModule,flatFrame};auto flat=SpanRecipe(flatModule,flatFrame,6,-18,18);
