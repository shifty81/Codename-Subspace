...
# Run native acceptance suites (Pass295 baseline: 4,301 assertions / 0 failed)
./build/subspace_tests       # Linux / macOS
.\build\subspace_tests.exe   # Windows (PowerShell)
./build/subspace_milestone_tests       # milestone acceptance (Linux/macOS)
.\build\subspace_milestone_tests.exe # milestone acceptance (Windows)
./build/subspace_production_tests      # Pass266-295 production integration (Linux/macOS)
.\build\subspace_production_tests.exe # Pass266-295 production integration (Windows)
ctest --test-dir build --output-on-failure # runs all three suites
```
